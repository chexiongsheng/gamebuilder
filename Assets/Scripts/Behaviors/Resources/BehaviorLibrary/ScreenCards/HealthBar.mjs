...
export function onDrawScreen() {
  ...
  uiRect(getProps().X, getProps().Y, getProps().Width, HEIGHT, barBgColor, {
    opacity: getProps().Opacity || 0.5
  });
  ...
  uiRect(getProps().X + MARGIN, getProps().Y + MARGIN + 35, barWidth, 10, bgColor);
  uiRect(getProps().X + MARGIN, getProps().Y + MARGIN + 35, filledBarWidth, 10, fgColor);
  uiText(getProps().X + getProps().Width - MARGIN - 40, getProps().Y + MARGIN, cur, fgColor);
}