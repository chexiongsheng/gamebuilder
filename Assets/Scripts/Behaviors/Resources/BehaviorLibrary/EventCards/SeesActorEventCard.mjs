...
export function onTick() {
  const eyePos = getPointAbove(getProps().EyeHeight || 1);
  for (const target of getActorsInGroup(getProps().Targets, getProps().Range)) {
    const toTarget = getPos(target).sub(eyePos);
    toTarget.normalize();
    const forward = getForward();
    const degreesOff = radToDeg(forward.angleTo(toTarget));
    const isInViewCone = degreesOff < getProps().ViewConeDegrees / 2;
    ...
    // the way).
    const closestHit = castAdvanced(eyePos, toTarget, getProps().Range, 0, CastMode.CLOSEST, true, false, true);
...
export function getCardStatus() {
  return {
    description: `When I see <color=yellow>${getActorGroupDescription(getProps().Targets, true)}</color> within distance <color=green>${getProps().Range.toFixed(1)}</color>`
  }
}