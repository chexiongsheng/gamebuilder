...
class ModuleBehaviorSystem {
  ...
  // BehaviorSystem.cs 里的CallBehaviorUseMethod发送消息触发，puerts可以修改成Delegate来调用
  callBehaviorUseMethod(useId, actorId, methodName, args) {
    ...
    // console.error(`call module: ${use.behavior.getModuleKey()} ${new Error().stack}`);
    ...
    while (!this.messageQueue_.isEmpty()) {
      ...
      if (item.toActorName) {
        if (!this.doesActorExist(item.toActorName)) {
          ...
          if (actor.brainName === undefined) {
            ...
            if (actor.isLocalActor || deliverLocallyEvenIfRemote) {
              //console.error(`handleMessage1: ${JSON.stringify(deliveredMessage)} ${new Error().stack}`);
        ...
        this.forActorsHandling_(item.messageName, actor => {
          ...
          if (shouldSend) {
            if (actor.isLocalActor || deliverLocallyEvenIfRemote) {
              //console.error(`handleMessage2: ${JSON.stringify(deliveredMessage)} ${new Error().stack}`);
              actor.handleMessage(deliveredMessage);
            }
            else if (shouldRemoteForward) {
        ...
        if (isResetGame) {
          this.forActorsHandling_("Init", actor => {
            if (actor.isLocalActor) {
              ...
              //console.error(`handleMessage3: ${JSON.stringify(deliveredMessage)} ${new Error().stack}`);
...
exports.MAX_ACTORS = MAX_ACTORS;
exports.MESSAGES_THAT_SHOULD_NOT_BE_FORWARDED = MESSAGES_THAT_SHOULD_NOT_BE_FORWARDED;
exports.MESSAGES_TO_DELIVER_LOCALLY_TO_REMOTE_ACTORS = MESSAGES_TO_DELIVER_LOCALLY_TO_REMOTE_ACTORS;
exports.ModuleBehaviorSystem = ModuleBehaviorSystem;
exports.getBehaviorProperties = getBehaviorProperties;
