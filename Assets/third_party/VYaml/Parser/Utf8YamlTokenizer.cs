...
namespace VYaml.Parser
{
  ...
  {
    public YamlTokenizerException(in Marker marker, string message) : base($"{message} at {marker}")
    {
    }
  }
  ...
  public ref struct Utf8YamlTokenizer
  {
    ...
    void ConsumeNextToken()
    {
      ...
      switch (currentCode)
      {
        ...
        case YamlCodes.BlockEntryIndent when !TryPeek(1, out var nextCode) ||
          ...
          ConsumeBlockEntry();
          break;
        case YamlCodes.ExplicitKeyIndent when !TryPeek(1, out var nextCode2) ||
                                                YamlCodes.IsEmpty(nextCode2):
        ...
        case YamlCodes.BlockEntryIndent when !TryPeek(1, out var nextCode) ||
          ...
          ConsumePlainScalar();
          break;
        case YamlCodes.MapValueIndent when flowLevel == 0 &&
                         (!TryPeek(1, out var nextCode3) || YamlCodes.IsBlank(nextCode3)):
        case YamlCodes.ExplicitKeyIndent when flowLevel == 0 &&
                         (!TryPeek(1, out var nextCode4) || YamlCodes.IsBlank(nextCode4)):
          ConsumePlainScalar();
          break;
        case (byte)'%':
        case (byte)'@':
    ...
    bool TryConsumeUriChar(Scalar scalar)
    {
      if (currentCode == '%')
      {
        ...
        scalar.Write(new ReadOnlySpan<byte>(toWrite, 0, width));
        return true;
      }
    ...
    // TODO: Use Uri
    int ConsumeUriEscapes(Span<byte> span)
    {
      var width = 0;
      var index = 0;
      while (!reader.End)
    ...
    void ConsumeBlockScaler(bool literal)
    {
      ...
      if (currentCode == (byte)'+' || currentCode == (byte)'-')
      {
